/*. Escreva um programa que insira valores uma lista. Você pode permitir a remoção do primeiro
elemento da lista. Quando isso ocorrer, esse elemento deve ser inserido em uma pilha e removido
da lista. Você deve permitir ao usuário imprimir a lista e a pilha a qualquer momento.*/
...
void excluir_fim (lista** inicio, lista** lixo){
	lista *pos, *aux;
...
void insere_ordenado (lista** inicio){
	lista *pos, *ant, *novo;
	int x;
	...
	do{
		...
		switch(op){
			...
			case 4:
				...
				break;
			case 5:
				excluir_fim(&inicio, &lixo);
				break;
			case 6:
				excluir_meio(&inicio);
				break;
			case 7:
				insere_ordenado(&inicio);
				break;
		}
	}while (op != 8);
